...
const FoodTabs = () => {
  ...
  useEffect(() => {
    const fetchOutlets = async () => {
      try {
        const response = await api1.get('/food/getOutlets');
  ...
  useEffect(() => {
    if (selectedOutlet) {
      const fetchMenus = async () => {
        try {
          const response = await api1.get(`/food/getFoodMenus?outletId=${selectedOutlet}`);
                ...
                menus.map((menu) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xl text-white mb-4">Price: Php {menu.Price}</p>
                    </div>
                    {/* Quantity controls */}
